...
Phase 5.2: Barrel-based xHR prior implementation - FIXED NULL CONSTRAINTS
"""
import sqlite3
import pandas as pd
import os
...
def map_statcast_row_to_db(row):
    ...
    mapped = {
        ...
        'half_inning': (row['inning_topbot'].upper() if not pd.isna(row['inning_topbot']) and row['inning_topbot'].upper() in ('TOP', 'BOT') else None),
...
def populate_statcast_table():
    ...
    # Define NOT NULL columns (based on table schema)
    NOT_NULL_COLUMNS = {
