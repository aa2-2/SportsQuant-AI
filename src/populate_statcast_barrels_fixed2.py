...
Phase 5.2: Barrel-based xHR prior implementation - FIXED HALF_INNING
"""
import sqlite3
import pandas as pd
import os
...
def map_statcast_row_to_db(row):
    ...
    Only use columns we know exist in the CSV
    """
    # Map column names from CSV to database
    inning_topbot_val = row['inning_topbot'] if not pd.isna(row['inning_topbot']) else None
    # Normalize half_inning to uppercase and only allow 'TOP' or 'BOT'
    half_inning = None
    if inning_topbot_val is not None:
        val_upper = str(inning_topbot_val).upper()
        if val_upper in ('TOP', 'BOT'):
            half_inning = val_upper
    ...
    mapped = {
        ...
        # launch_direction is not in CSV, set to NULL
        ...
        # babip: we have babip_value in CSV
        'babip': float(row['babip_value']) if not pd.isna(row['babip_value']) else None,
        # slg: not in CSV, set to NULL
        'slg': None,
        # woba_value and woba_denom are in CSV
